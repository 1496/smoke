(function(window, $) {
    ...
    G.getType = function(e)
    {
        ...
        {
            PLAYER.fullScreenEffects[key](key);
        }
        //オートモード
        else if(key == 'enter')
        {
        ...
        {
            if(1200 < PLAYER.autoPlayInterval)
            {
                PLAYER.autoPlayInterval -= 1000;
                PLAYER.fullScreenEffects['enter']('enter');
            }
        }
        //オートモード時間の変更(遅くする)
        else if(key == 'speedDown')
        {
            if(PLAYER.autoPlayInterval < 10000)
            {
                PLAYER.autoPlayInterval += 1000;
                PLAYER.fullScreenEffects['enter']('enter');
            }
